...
## 1.5 파싱 ↔ 온톨로지 연결 — 이 프로젝트만의 강점

> 이 프로젝트는 이미 PDF에서 **구조화된 요소**(텍스트/표/이미지/목차/링크/주석)를
> 분리 추출한다. 대부분의 지식그래프 파이프라인이 "평문 덩어리"에서 시작하는 것과 달리,
> 우리는 **파싱 단계의 구조 정보를 온톨로지 추출의 힌트로 그대로 쓸 수 있다.**
> 이것이 "데이터 파싱과 온톨로지를 엮는" 핵심 설계다.

### 1.5.1 파싱 산출물 → 온톨로지 매핑 표

현재 파서(`parser.py`)가 만드는 모든 산출물은 온톨로지의 특정 클래스/관계로 흘러들어간다:

| 파싱 산출물 (현재 DB 테이블) | 온톨로지에서의 역할 | 추출 규칙 (구상) |
|---|---|---|
| `documents.metadata_json` (제목/저자/생성일) | `Document` 노드 + **`Person`(저자), `Date`(생성일) 노드가 공짜** | 파싱만으로 확보 — NER 불필요, 신뢰도 1.0 |
| `text_chunks` (본문 청크) | 엔티티/관계 추출의 **주 입력** | GLiNER/LLM 추출 대상. `mentions` 엣지는 chunk_id로 페이지까지 추적 |
| `tables` (표: 헤더+셀) | **`Metric` 노드의 보고(寶庫)** — "매출, 3조원" 같은 (개념, 수치) 쌍이 이미 행/열로 정렬됨 | 헤더 셀 = `Concept` 후보, 데이터 셀 = `Metric` 후보, 같은 행 = `measured_as` 관계 후보. **평문 NER보다 훨씬 높은 정밀도** |
| `outlines` (목차) | **`Concept` 계층이 공짜** — 목차의 들여쓰기(level)가 곧 SKOS broader/narrower | "3. 리스크 관리 > 3.1 유동성 리스크" → `유동성리스크 —part_of→ 리스크관리`, 신뢰도 높음 (저자가 직접 만든 구조) |
| `links` (하이퍼링크) | **문서 간 `references` 관계가 공짜** | URL이 다른 수집 문서를 가리키면 Document↔Document 엣지. 외부 URL은 출처 노드 |
| `annotations` (형광펜/메모) | **사람이 중요 표시한 스팬** = 엔티티 추출 우선순위 + 신뢰도 부스트 | 주석 걸린 청크에서 나온 엔티티는 가중치 ↑ (사용자의 암묵적 큐레이션) |
| `images` + 주변 캡션 | 이미지가 어떤 `Concept`을 도해하는지 연결 (`depicts` 관계 후보) | CLIP 임베딩 vs Concept 라벨 임베딩 유사도 → 낮은 신뢰도로 연결 |
| `page_number` (모든 요소) | **provenance의 최소 단위** | 모든 트리플이 "문서 X의 p.Y에서 추출됨"을 유지 → 검증/감사 가능 |

**요점: 파서가 이미 만들어 놓은 구조가 온톨로지 추출의 절반을 해결한다.**
메타데이터·목차·표·링크에서 나오는 노드/엣지는 NER 없이 규칙만으로 뽑히고 신뢰도도 높다.
→ P2(GLiNER) 이전에 **"P1.5: 구조 기반 추출(파싱→온톨로지 직결)"** 단계를 먼저 넣을 가치가 있다.
LLM/NER 0회 호출로 첫 엔티티 그래프가 나온다.

### 1.5.2 역방향: 온톨로지가 파싱을 개선한다 (schema-aware parsing)

연결은 단방향이 아니다. 온톨로지가 있으면 파서 자체가 똑똑해진다:

1. **표 헤더 정규화** — 온톨로지에 `Metric`의 알려진 단위(원, %, 건)를 정의해두면,
   파서가 표에서 "3,000 (백만원)" 같은 셀을 수치+단위로 정규화 가능
2. **문서 타입 분류** — 온톨로지에 `Document`의 하위 타입(보고서/논문/계약서/매뉴얼)을 두면,
   목차 패턴·메타데이터로 문서 타입을 분류하고 타입별 파싱 전략 적용
   (예: 논문이면 참고문헌 섹션을 `references` 관계로 특별 처리)
3. **청킹 개선** — 목차(outline) 경계를 청크 경계로 쓰면 "섹션 = 의미 단위"가 보장되고,
   각 청크가 어느 `Concept`(섹션 주제) 아래 있는지 문맥이 붙는다
   → 엔티티 추출 시 "이 청크는 '리스크 관리' 섹션임"이라는 힌트 제공
4. **파싱 품질 지표** — 하네스(§3)에서 "온톨로지 매핑 가능한 요소 비율"을
   파서 품질 지표로 사용 (표를 못 뽑으면 Metric도 없다 — 파싱 회귀가 그래프 품질 회귀로 드러남)

### 1.5.3 통합 파이프라인 (목표 아키텍처)

```
PDF ──parser.py──> 구조화 산출물 ─┬─ [규칙 추출] 메타/목차/표/링크 ──> 고신뢰 노드·엣지 (P1.5)
                                  ├─ [GLiNER]   text_chunks ────────> 엔티티 후보 + 확률 (P2)
                                  └─ [LLM(선택)] 관계 정제 ─────────> 타입 있는 관계 (P7)
                                                     │
                              온톨로지(LinkML) ──────┤  ← 추출 스키마 제약
                              SHACL 검증 ────────────┤  ← 위반 트리플 거부
                              신뢰도 결합(§2) ───────┘
                                                     ▼
                            .pdfsearch/db.sqlite (entities/mentions/relations)
                                                     ▼
                        기존 FAISS 벡터 검색 + 그래프 검색 = 하이브리드 (LightRAG식)
```

파싱(구조) → 온톨로지(의미) → 검색(활용)이 하나의 흐름이 되고,
각 단계가 다음 단계의 품질을 끌어올리는 구조다.
...
| **P1.5** | **구조 기반 추출** — 파싱 산출물(메타/목차/표/링크)을 온톨로지 노드·엣지로 직결 (§1.5) | 규칙 기반, LLM/NER 불필요 | 고신뢰 1차 그래프 | ★ |
